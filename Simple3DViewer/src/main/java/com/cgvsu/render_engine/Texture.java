...
public class Texture {
    ...
    // Слой рисунка (прозрачный), очищается кнопкой Clear
    private final WritableImage paintLayer;
    private final PixelReader paintReader;
    private final PixelWriter paintWriter;

    // Итоговая (base + paint), именно её семплим в рендере и показываем в UI
    ...
    public void clearPaintLayer() {
        // очищаем paintLayer до прозрачного и восстанавливаем composite = base
    ...
    private void applyPaintPixel(final int x, final int y, final Color brushColor) {
        ...
        // 1) обновляем paintLayer (накладываем кисть на существующий оверлей)
        ...
        // 2) обновляем composite = base + newOverlay
